...
Процедура КоммунальныеУслугиУслугаНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	ЗначенияОтбора = Новый Структура("Ссылка",
			ОбщийМодульПокупкиВызовСервера.ПолучитьНоменклатуру(ПредопределенноеЗначение("Перечисление.ВидНоменклатуры.КоммунальныеПлатежи")));
...
Процедура КоммунальныеУслугиУслугаАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	Если СтрДлина(Текст)>2 Тогда 
		...
		СписокУслуг = ОбщийМодульПокупкиВызовСервера.ПолучитьНоменклатуру(
				ПредопределенноеЗначение("Перечисление.ВидНоменклатуры.КоммунальныеПлатежи"),"%" + Текст + "%");
...
&НаКлиенте
Процедура КоммунальныеУслугиПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	ПодсчетСуммыДокумента();
КонецПроцедуры
#КонецОбласти
